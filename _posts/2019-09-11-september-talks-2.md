...
用時十個月，我開始體會到為什麼蝴蝶鍵盤時常被人吐槽了。

## 珠排序

珠排序一點也不靠譜，不說準備時間沒有算進複雜度，不計資源的話歸併還能 $O(\log n)$ 完成呢。

## Apple Watch

入手了水果表，準備寫一個 Heartbeat Self-check System。畢竟我有一個會把備忘錄忘掉的腦子。
...
動態網頁由 JS 加載，須模擬調用請求，或分析 ajax 請求，如此循環直到獲得包含數據的 json。

反蜜罐，避開隱含元素。

迷惑性的 URL 不會帶太多數據，只須按數據量倒序分析即可。
